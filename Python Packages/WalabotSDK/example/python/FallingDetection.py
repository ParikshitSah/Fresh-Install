...
if __name__ == '__main__':
    height = input('Please enter height of the room (distance of sensor from floor) in cm: ')
    fallDetectorAgent = FallDetectionAgent(float(height), 50, 5, 10)
    fallDetectorAgent._detector.FallDetectionInit()
    while True:
        fallDetectorAgent._detector.FallDetectionCheck(fallDetectorAgent.height, fallDetectorAgent.Fall_threshold, fallDetectorAgent.Position_confidenceFactor, fallDetectorAgent.framesToIndicateFall)
